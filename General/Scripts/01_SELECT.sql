 ...
 * 1>> 특정 테이블에서
 * 2>> 조건식을 만족하는 행을 추려놓고
 * 3>> 추려진 결과 행(RESULT SET)에서 원하는 컬럼만 조회
 */
...
WHERE SALARY >= 4000000
	AND SALARY <= 6000000; -- 6행 조회

-- 2) BETWEEN 버전 
SELECT EMP_NAME, SALARY 
FROM EMPLOYEE
WHERE SALARY BETWEEN 4000000 AND 6000000; -- 6행 조회
...
SELECT EMP_NAME, SALARY 
FROM EMPLOYEE
WHERE SALARY NOT BETWEEN 4000000 AND 6000000; -- 17행 조회
...
-- 부서코드 오름차순, 직급코드 내림차순, 이름 오름차순으로 정렬해서 조회
SELECT 
		EMP_NAME 이름, 
		DEPT_CODE 부서코드, 
		JOB_CODE 직급코드
