...
/* TRANSACTION이란?
 - 데이터베이스의 논리적 연산 단위
	...
	DELETE			| 시	|					 |     	    																  |
							|	저	|			 		 |				  																  |
							|	장	|			 		 |____________________________________________|
							|	  |		
					<=	|___|			<=						       SELECT
...
--  SELECT시 조회 결과에 INSERT 내용이 포함된다!
...
SELECT * FROM DEPARTMENT3; --COMMIT 하지 않아도 데이터베이스에서 SELECT로 조회 가능!

/* 트랜잭션에 저장된 내용(INSERT 1개, UPDATE 1개)을 삭제*/
ROLLBACK;
SELECT * FROM DEPARTMENT3;

-- 다시 INSERT, UPDATE 수행 진행 후 아래 COMMIT 구문 실행!!
...
SELECT * FROM DEPARTMENT3; -- 롤백을 해도 그대로 변경사항이 저장되어있다
...
-- SAVEPOINT "SP1" 저장 지점 설정 -- D0삭제 -> SP1
SAVEPOINT "SP1";
...
-- SAVEPOINT "SP2" 저장 지점 설정 -- D9삭제 -> SP2
SAVEPOINT "SP2";
...
-- SAVEPOINT "SP3" 저장 지점 설정  -- D8삭제 -> SP3
SAVEPOINT "SP3";
