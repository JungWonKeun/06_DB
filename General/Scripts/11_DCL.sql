 ...
 * 사용자 계정 :	 데이터베이스에 대해여 질의, 갱신, 보고서 작성 등의
  						 작업을 수행할 수 있는 계정으로
  						 업무에 필요한 최소한의 권한만을 가지는 것을 원칙으로 한다.
  					   ex) KH_jwk계정(각자 이니셜 계정), workbook 등
 */
 ...
 * 
 * RESOURCE(자원) : 8개 객체 권한을 묶어둔 ROLE
 * 
 * CONNECT(접속)  : DB 접속 권한
 *  
 */
...
SELECT * FROM kh_JWK.EMPLOYEE;

-- 다른 테이블 조회도 되는지 확인
SELECT * FROM kh_JWK.DEPARTMENT;
...
SELECT * FROM kh_JWK.EMPLOYEE; -- 조회 불가
