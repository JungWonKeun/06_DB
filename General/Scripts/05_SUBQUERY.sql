...
-- 사번, 이름, 부서코드, 직급코드, 부서명, 직급명
SELECT
		EMP_ID, EMP_NAME, DEPT_CODE, DEPT_TITLE, JOB_NAME
FROM EMPLOYEE
JOIN JOB USING (JOB_CODE)
JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
WHERE (DEPT_CODE,JOB_CODE) =
...
SELECT
		EMP_ID,
		EMP_NAME,
		NVL(DEPT_TITLE, '소속없음') AS DEPT_TITLE,
		JOB_NAME,
		HIRE_DATE
FROM EMPLOYEE E1
LEFT JOIN 
		DEPARTMENT ON (DEPT_CODE = DEPT_ID)
JOIN 
		JOB J ON (E1.JOB_CODE = J.JOB_CODE)
WHERE HIRE_DATE IN 
	(SELECT MIN(HIRE_DATE)
	FROM EMPLOYEE E2
	WHERE ENT_YN != 'Y'
	AND NVL(E2.DEPT_CODE, '소속 없음') 
		= NVL(E1.DEPT_CODE, '소속 없음'))
...
SELECT
    ...
    FLOOR(
    		MONTHS_BETWEEN(
    			CURRENT_DATE, 
    			TO_DATE(SUBSTR(EMP_NO, 1, 6))) / 12) AS "만 나이",
